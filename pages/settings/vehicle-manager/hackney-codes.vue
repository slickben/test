    ...
    <div class="flex h-full xl:mr-24">
            ...
            <SettingsTable :head_data="table_head_data">
                <template slot="head">
                    <div class="flex items-center justify-between pb-6 xl:py-10">
                        <h3 class="text-xl font-medium text-primary-900">Hackney Codes</h3>
                        <div class="flex">
                            <PrimaryButton :onClick="toggleAddFunc" title="Add Hackney Codes" type="solid" />
                ...
                <tr v-for="hackney_code in hackney_codes" class="hover:bg-primary-100 group ">
                    <td class="py-5 px-4"> {{ hackney_code.name }} </td>
                    <td class="py-5 px-4"> {{ hackney_code.code }} </td>
                    <td class="py-5 px-4 group-hover:text-tertiary-400 text-transparent flex justify-end">
                        <div class="flex items-center">
                            <button @click="toggleEditFunc(hackney_code.name)" class="flex items-center focus:outline-none pr-2 opacity-0 group-hover:opacity-100">
                                <img src="~/assets/icons/edit.svg" alt="" srcset="">
                                <p class="text-xs font-normal pl-1 text-primary-500">Edit</p>
                            </button>
                            <button @click="deleteHackneyCode(hackney_code.id)" class="flex items-center focus:outline-none pr-2 opacity-0 group-hover:opacity-100">
        ...
        <Sliding classes="min-w-105 w-full" v-show="toggle_add">
            <template slot="head">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-2xl text-primary-900 font-semibold">Add Hackney Codes</h4>
            ...
            <div>
                <form class="p-6 px-10 pt-16" @submit.prevent="submitAdd">
                    ...
                    <Input class="pb-6" v-model="hackney_code.name" type="text" id="name" lable="Name" place_holder="Enter Name" />
                    <Input v-model="hackney_code.code" type="number" id="name" lable="Code" place_holder="Enter Name" />
        ...
        <Sliding classes="min-w-105 w-full" v-show="toggle_edit">
            <template slot="head">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-2xl text-primary-900 font-semibold">Edit Hackney Codes</h4>
                        <!-- <p class="text-base text-tertiary-600 font-normal py-2">#0123</p/> -->
                    </div>
...
export default {
    ...
    methods: {
        ...mapActions({
            addHackneyCode: 'settings/vehicle_manager/addHackneyCode',
            deleteHackneyCode: 'settings/vehicle_manager/deleteHackneyCode',
            editHackneyCode: 'settings/vehicle_manager/editHackneyCode',
        }),
        toggleAddFunc () {
            this.toggle_add = !this.toggle_add
            this.hackney_code.name = ''
            this.hackney_code.code = ''
        },
        toggleEditFunc (name) {
            this.toggle_edit = !this.toggle_edit
            this.hackney_codes.map( hackney_code => {
                if(hackney_code.name === name) {
                    this.hackney_code.name = hackney_code.name
                    this.hackney_code.code = hackney_code.code
                    this.selected_id = hackney_code.id
                }
            })
        },
        submitAdd () {
            this.addHackneyCode(this.hackney_code)
        },
        submitEdit () {
            this.editHackneyCode({hackney_code: this.hackney_code, id: this.selected_id})
        }
    },
    computed: mapState({

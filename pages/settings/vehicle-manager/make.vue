    ...
    <div class="flex h-full xl:mr-24">
            ...
            <SettingsTable :head_data="table_head_data">
                <template slot="head">
                    <div class="flex items-center justify-between pb-6 xl:py-10">
                        <h3 class="text-xl font-medium text-primary-900">Vehicle Makes</h3>
                        <div class="flex">
                            <PrimaryButton :onClick="toggleAddFunc" title="Add Vehicle Makes" type="solid" />
                            <!-- <PrimaryButton title="Import Vehicle Makes" /> -->
                        </div>
                    </div>
                ...
                <tr v-for="make in makes" class="hover:bg-primary-100 group ">
                    <td class="py-5 px-4"> {{ make.name }} </td>
                    <td class="py-5 px-4 group-hover:text-tertiary-400 text-transparent flex justify-end">
                        <div class="flex items-center">
                            <button @click="toggleEditFunc(make.name)" class="flex items-center focus:outline-none pr-2 opacity-0 group-hover:opacity-100">
                                <img src="~/assets/icons/edit.svg" alt="" srcset="">
                                <p class="text-xs font-normal pl-1 text-primary-500">Edit</p>
                            </button>
                            <button @click="deleteMake(make.id)" class="flex items-center focus:outline-none pr-2 opacity-0 group-hover:opacity-100">
        ...
        <Sliding classes="min-w-105 w-full" v-show="toggle_add">
            <template slot="head">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-2xl text-primary-900 font-semibold">Add Vehicle Make</h4>
            ...
            <div>
                <form class="p-6 px-10 pt-16" @submit.prevent="submitAdd">
                    ...
                    <Input v-model="make.name" type="text" id="name" lable="Name" place_holder="Enter Name" />
        ...
        <Sliding classes="min-w-105 w-full" v-show="toggle_edit">
            <template slot="head">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-2xl text-primary-900 font-semibold">Edit Vehicle Make</h4>
                        <!-- <p class="text-base text-tertiary-600 font-normal py-2">#0123</p/> -->
                    </div>
...
export default {
    ...
    methods: {
        ...mapActions({
            addMake: 'settings/vehicle_manager/addMake',
            deleteMake: 'settings/vehicle_manager/deleteMake',
            editMake: 'settings/vehicle_manager/editMake',
        }),
        toggleAddFunc () {
        ...
        toggleEditFunc (name) {
            ...
            this.makes.map( make => {
                if(make.name === name) {
                    this.make.name = make.name
                    this.selected_id = make.id
                }
            })
        },
        submitAdd () {
            this.addMake(this.make)
        },
        submitEdit () {
            this.editMake({make: this.make, id: this.selected_id})
        }
    },
    computed: mapState({

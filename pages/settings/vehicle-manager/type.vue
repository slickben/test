    ...
    <div class="flex h-full xl:mr-24">
            ...
            <SettingsTable :head_data="table_head_data">
                <template slot="head">
                    <div class="flex items-center justify-between pb-6 xl:py-10">
                        <h3 class="text-xl font-medium text-primary-900">Vehicle Type</h3>
                        <div class="flex">
                            <PrimaryButton :onClick="toggleAddFunc" title="Add Type" type="solid" />
                ...
                <tr v-for="v_type in v_types" class="hover:bg-primary-100 group ">
                    <td class="py-5 px-4"> {{ v_type.name }} </td>
                    <td class="py-5 px-4 group-hover:text-tertiary-400 text-transparent flex justify-end">
                        <div class="flex items-center">
                            <button @click="toggleEditFunc(v_type.name)" class="flex items-center focus:outline-none pr-2 opacity-0 group-hover:opacity-100">
                                <img src="~/assets/icons/edit.svg" alt="" srcset="">
                                <p class="text-xs font-normal pl-1 text-primary-500">Edit</p>
                            </button>
                            <button @click="deleteVehicleType(v_type.id)" class="flex items-center focus:outline-none pr-2 opacity-0 group-hover:opacity-100">
        ...
        <Sliding classes="min-w-105 w-full" v-show="toggle_add">
            <template slot="head">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-2xl text-primary-900 font-semibold">Add Type</h4>
            ...
            <div>
                <form class="p-6 px-10 pt-16" @submit.prevent="submitAdd">
                    ...
                    <Input v-model="v_type.name" type="text" id="name" lable="Name" place_holder="Enter Name" />
        ...
        <Sliding classes="min-w-105 w-full" v-show="toggle_edit">
            <template slot="head">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-2xl text-primary-900 font-semibold">Edit Type</h4>
                        <!-- <p class="text-base text-tertiary-600 font-normal py-2">#0123</p/> -->
                    </div>
...
export default {
    ...
    methods: {
        ...mapActions({
            addVehicleType: 'settings/vehicle_manager/addVehicleType',
            deleteVehicleType: 'settings/vehicle_manager/deleteVehicleType',
            editVehicleType: 'settings/vehicle_manager/editVehicleType',
        }),
        toggleAddFunc () {
        ...
        toggleEditFunc (name) {
            ...
            this.v_types.map( v_type => {
                if(v_type.name === name) {
                    this.v_type.name = v_type.name
                    this.selected_id = v_type.id

                }
            })
        },
        submitAdd () {
            this.addVehicleType(this.v_type)
        },
        submitEdit () {
            this.editVehicleType({type: this.v_type, id: this.selected_id})
        }
    },
    computed: mapState({
